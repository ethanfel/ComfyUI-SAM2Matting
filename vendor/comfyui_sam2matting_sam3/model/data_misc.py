...
class BatchedFindTarget:
    ...
    alpha_segments: Optional[MyTensor]
    alpha_segments__type = torch.float
    
    # Trimap (float, {0,1}), transition region ground-truth
    trimap_segments: Optional[MyTensor]
    trimap_segments__type = torch.float
    
    # Foreground image ([3,H,W], float [0,1])
    fg_segments: Optional[MyTensor]
    fg_segments__type = torch.float
