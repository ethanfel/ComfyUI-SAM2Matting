...
from comfyui_sam2matting_sam3.model.memory import SimpleMaskEncoder

from comfyui_sam2matting_sam3.model.sam3_tracker_utils import get_1d_sine_pe, select_closest_cond_frames

from comfyui_sam2matting_sam3.sam.mask_decoder import MaskDecoder, MLP
from comfyui_sam2matting_sam3.sam.prompt_encoder import PromptEncoder
from comfyui_sam2matting_sam3.sam.transformer import TwoWayTransformer
# from comfyui_sam2matting_sam3.train.data.collator import BatchedDatapoint
from comfyui_sam2matting_sam3.model.data_misc import BatchedDatapoint
...
class Sam3TrackerBase(torch.nn.Module):
    ...
    def forward_tracking(self, backbone_out, input, return_dict=False):
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                num_objects = backbone_out["mask_inputs_per_frame"].get(0, None).shape[0]
                assert num_objects > 0
                if num_objects > 1:
                    current_vision_feats = [feat.expand(-1, num_objects, -1) for feat in current_vision_feats]
                    current_vision_pos_embeds = [pos.expand(-1, num_objects, -1) for pos in current_vision_pos_embeds]
            else:
            ...
            high_res_masks = mask_inputs.float() * out_scale + out_bias
            low_res_masks = F.interpolate(
                high_res_masks,
                size = (288,288),
                mode="bilinear",
            ...
            current_matting_output = self.matting_step(
                frame_idx=stage_id,
                input = input.img_batch,
                ...
                matting_output_dict=matting_output_dict,
                num_frames=num_frames,
            )
            if add_output_as_cond_frame:
                matting_output_dict["cond_frame_outputs"][stage_id] = current_matting_output
            else:
                matting_output_dict["non_cond_frame_outputs"][stage_id] = current_matting_output
            del current_matting_output
            ###############################    
        ...
        #########################
        # SAM3Matting
        all_frame_outputs.update(matting_output_dict["cond_frame_outputs"])
        all_frame_outputs.update(matting_output_dict["non_cond_frame_outputs"])
        #########################
    ...
    def track_step(
        ...
        current_out["pred_masks"] = low_res_masks # (1,1,288,288)
        current_out["pred_masks_high_res"] = high_res_masks # (1,1,1008,1008)
        ...
        if run_mem_encoder and self.num_maskmem > 0:
            ...
            current_out["pix_feat"] = pix_feat  # (B, C, H_feat, W_feat)
            current_out["maskmem_backbone"] = self.maskmem_backbone
            current_out["feat_sizes"] = feat_sizes
    ...
    def _compile_all_components(self):
        ...
        from comfyui_sam2matting_sam3.perflib.compile import compile_wrapper
    ...
    def matting_step(
        ...
        current_out, alpha_outputs = self._matting_step(
            frame_idx,
            input,
            mask_inputs,
            current_vision_feats,
        ...
        _, low_res_alpha, high_res_alpha, unknown_region_mask = alpha_outputs
        current_out["multistep_pred_unknown_region"] = [unknown_region_mask]
        current_out["multistep_pred_alphas"] = [low_res_alpha]
        current_out["multistep_pred_alphas_high_res"] = [high_res_alpha]
        return current_out
    ...
    def _matting_step(
        ...
        alpha_outputs = self._forward_alpha_heads(
            ...
            point_inputs=point_inputs,
            mask_inputs=mask_inputs,
            unknown_region_inputs=unknown_region_inputs,
            high_res_features=high_res_features,
        )
    ...
    def _forward_alpha_heads(
        ...
        if len(img.shape) == 3:
            img = img.unsqueeze(0).repeat(mask_inputs.shape[0], 1, 1, 1)
    ...
    def _detect_unknown_region(self, high_res_features, mask_inputs, img):
        ...
        unknown_region = unknown_region_logits  
        return unknown_region, img_scale_2, img_scale_4, img_scale_8
